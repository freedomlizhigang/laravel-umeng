...
use LiZG\UmengPush\Umeng\Android\AndroidBroadcast;
use LiZG\UmengPush\Umeng\Android\AndroidFilecast;
use LiZG\UmengPush\Umeng\Android\AndroidGroupcast;
use LiZG\UmengPush\Umeng\Android\AndroidUnicast;
use LiZG\UmengPush\Umeng\Android\AndroidCustomizedcast;
use LiZG\UmengPush\Umeng\Ios\IOSBroadcast;
use LiZG\UmengPush\Umeng\Ios\IOSFilecast;
use LiZG\UmengPush\Umeng\Ios\IOSGroupcast;
use LiZG\UmengPush\Umeng\Ios\IOSUnicast;
use LiZG\UmengPush\Umeng\Ios\IOSCustomizedcast;
...
class UmengPush
{
	...
	function __construct($appkey = '',$appMasterSecret = '') {
		$this->appkey = config('umengpush.appkey');
		$this->appMasterSecret = config('umengpush.appMasterSecret');
	...
	function sendIOSUnicast($dtoken,$num) {
		try {
			...
			$unicast->setPredefinedKeyValue("timestamp", $this->timestamp);
			// Set your device tokens here
			$unicast->setPredefinedKeyValue("device_tokens",$dtoken); 
			$unicast->setPredefinedKeyValue("alert", '');
			$unicast->setPredefinedKeyValue("content-available", 1);
			$unicast->setPredefinedKeyValue("badge", $num);
			// $unicast->setPredefinedKeyValue("sound", "chime");
			// Set 'production_mode' to 'true' if your app is under production mode
			$unicast->setPredefinedKeyValue("production_mode", "true");
			// Set customized fields
			// $unicast->setCustomizedField("test", "");
			// print("Sending unicast notification, please wait...\r\n");
			$unicast->send();
			print("Sent SUCCESS\r\n");
		} catch (\Exception $e) {
